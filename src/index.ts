...
if (process.env.NODE_ENV !== 'production') dotenv.config({path: './.env.development'}); else dotenv.config();
import {approveSubmissions, checkSubmissionsLastUpdate} from './server/logic/tasks/submissionsTasks';
import {updateFavorites} from './server/logic/tasks/beatmapsTaks';
...
const setup = async (): Promise<void> => {
	if (!process.env.DATABASE_URL || !process.env.DEFAULT_USERNAME || !process.env.DEFAULT_PASSWORD || !process.env.SECRET_KEY ||
		!process.env.AWS_BUCKET_NAME || !process.env.AWS_BUCKET_REGION || !process.env.AWS_ACCESS_KEY || !process.env.AWS_SECRET_KEY) {
	...
	if (process.env.BOT_USERNAME && process.env.BOT_PASSWORD) {
		...
		client.on('PM', process.env.NODE_ENV !== 'production' ? (message: any) => {
			if (message.self) return;
			commandProcessing(message.message).then(response => console.info(response)).catch(error => console.error(error));
		} : async (message: any) => {
			if (message.self) return;
			try {
