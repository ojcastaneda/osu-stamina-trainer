...
import OsuBeatmapset from '../models/osu!/beatmapset';
import OsuBeatmap from '../models/osu!/beatmap';
...
class OsuService {
	...
	public retrieveRankedBeatmaps = async (lastDate: number, lastBeatmapset: number):
		Promise<[beatmaps: ProcessingBeatmap[], lastDate: number, lastBeatmapset: number, beatmapsLeft: boolean]> => {
		try{
			const beatmaps: ProcessingBeatmap[] = [];
			const params = [
				`m=0`,
				'sort=ranked_asc',
				'nsfw=true',
			...
			if (response.ok) {
				...
				const beatmapsets: OsuBeatmapset[] = [];
				for (let i = 0, length = json['beatmapsets'].length; i < length; i++)
					beatmapsets.push(new OsuBeatmapset(json['beatmapsets'][i]));
				const cursor = new Cursor(json['cursor']);
				if (cursor.lastDate < this.lastMonth) {
					for (let i = 0, length = beatmapsets.length; i < length; i++)
						this.includeBeatmaps(beatmaps, beatmapsets[i]);
					return [beatmaps, cursor.lastDate, cursor.lastBeatmapset, true];
				} else {
					for (let i = 0, length = beatmapsets.length; i < length; i++)
						if (beatmapsets[i].rankedDate != undefined && beatmapsets[i].rankedDate!.getTime() < this.lastMonth) {
							cursor.lastBeatmapset = beatmapsets[i].id;
							cursor.lastDate = beatmapsets[i].rankedDate!.getTime();
							this.includeBeatmaps(beatmaps, beatmapsets[i]);
						}
	 ...
	 * @return {Promise<(ProcessingBeatmap | undefined)>} The promise of the beatmap specified if available
	 */
	public retrieveBeatmap = async (id: number): Promise<ProcessingBeatmap | undefined> => {
		...
		if (response.ok) {
			...
			if (beatmap.stars > 3 && beatmap.mode === 'osu' && beatmap.beatmapset)
				return new ProcessingBeatmap(beatmap, beatmap.beatmapset);
		}
	};
	 ...
	 */
	private includeBeatmaps = (beatmaps: ProcessingBeatmap[], beatmapset: OsuBeatmapset) => {
		if (beatmapset.beatmaps instanceof Array && (beatmapset.ranked === 1 || beatmapset.ranked === 2))
			for (let i = 0, length = beatmapset.beatmaps.length; i < length; i++) {
				const beatmap = beatmapset.beatmaps[i];
				if ((beatmap.ranked === 1 || beatmap.ranked === 2) && beatmap.stars > 3 && beatmap.mode === 'osu')
					beatmaps.push(new ProcessingBeatmap(beatmap, beatmapset));
			}
	};
}
