...
class FileManager {
	...
	public getFileStream = (key: string): Readable => this.s3.getObject({ Key: key, Bucket: this.bucket }).createReadStream();
	...
	public setup = async () => {
		try {
			...
			if (process.env.NODE_ENV === 'testing') await this.uploadFile(JSON.stringify({ lastBeatmapset: 0, lastDate: 0 }), 'state.json');
		} catch (error) {
