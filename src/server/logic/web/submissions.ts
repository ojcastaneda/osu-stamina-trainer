import Submission, {
	Approved, createSubmission, deleteApprovedSubmission, retrieveSubmission, retrieveSubmissionsByPage, updateSubmission
...
const createSubmissionWeb = async (request: Request, response: Response, next: NextFunction) => {
	try {
		...
		if ((await retrieveBeatmap(id, ['id'])) !== undefined) return response.status(409).send('Beatmap already in the collection');
		const submission = await retrieveSubmission(id, ['id', 'approved']);
		if (submission === undefined) {
			await createSubmission({id});
			return response.status(204).send();
		}
		if (submission.approved !== 'pending') return response.status(409).send('Beatmap already in the collection');
...
/**
 * Changes the approved status to waiting_processing of an existing submission,
...
const approveSubmissionWeb = async (request: Request, response: Response, next: NextFunction) => {
	try {
		if (!(await updateSubmission({id: Number(request.params.id), approved: Approved.waiting_processing, last_updated: new Date()}))) {
			return response.status(404).
				send('Submission not found');
		}
		response.status(204).send();
	} catch (error) {
		next(error);
...
const retrieveSubmissionsByPageWeb = async (request: Request, response: Response, next: NextFunction) => {
	try {
		const {page, filters, sort}: { page: number; filters: Filter<Submission>[]; sort: Sort<Submission> } = request.body;
